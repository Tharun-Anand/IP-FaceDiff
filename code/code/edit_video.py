...
class TokenFlow(nn.Module):
    def __init__(self, config):
        ...
        model_key1 = "stabilityai/stable-diffusion-2-1-base"
        model_key2='CompVis/stable-diffusion-v1-2'
        model_key="rootkan/wucaicai-celeb-heywhale"
        #pipe = StableDiffusionPipeline.from_pretrained(model_key, torch_dtype=torch.float16).to("cuda")
        #pipe1=StableDiffusionPipeline.from_pretrained(model_key1, torch_dtype=torch.float16).to("cuda")
        pipe1=StableDiffusionPipeline.from_pretrained("/home/sushanth/deepfake_gen/Fast_Facial_Video_Editing/identity_model", torch_dtype=torch.float16).to("cuda")

        # pipe.enable_xformers_memory_efficient_attention()
        
        #model_path = "/mnt/data/tarun/finetune/epoch_7"
        #pipe1 = StableDiffusionPipeline.from_pretrained(pipe1, torch_dtype=torch.float16).to("cuda")
        self.vae = pipe1.vae
        self.tokenizer = pipe1.tokenizer
        self.text_encoder = pipe1.text_encoder
        self.unet = pipe1.unet
