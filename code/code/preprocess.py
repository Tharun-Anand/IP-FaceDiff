...
class Preprocess(nn.Module):
    def __init__(self, device, opt, hf_key=None):
        ...
        #model_key = "rootkan/wucaicai-celeb-heywhale"
        #model_key = "stabilityai/stable-diffusion-2-1-base"
        pipe = StableDiffusionPipeline.from_pretrained("/home/sushanth/deepfake_gen/Fast_Facial_Video_Editing/identity_model", torch_dtype=torch.float16)


        #pipeline = DiffusionPipeline.from_pretrained("svjack/face-control-lora-zh")
        ...
        if self.sd_version == 'ControlNet':
            ...
            #controlnet = ControlNetModel.from_pretrained("shiviguptta/sd-controllora-face-landmarks")
        ...
        self.scheduler = DDIMScheduler.from_pretrained("/home/sushanth/deepfake_gen/Fast_Facial_Video_Editing/identity_model", subfolder="scheduler")
    ...
    @torch.no_grad()
    def get_text_embeds(self, prompt, negative_prompt, device="cuda:0"):
    ...
    # load frames
        ...
        for frame in frames:
            ...
            faces = detector(frame_np)

            # If a face is detected, crop the frame to include only the face
            if faces:
        ...
        frames = [cropped_frame.resize((512, 512), resample=Image.Resampling.LANCZOS) for cropped_frame in cropped_frames]
