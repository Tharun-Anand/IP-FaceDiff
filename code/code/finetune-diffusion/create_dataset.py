...
if __name__ == '__main__':
    args = get_paramters()
    path_caption = "/home/sushanth/deepfake_gen/tokenflow/Tokenflow/finetune-diffusion/captions1_copy.json"
    path_data = "/mnt/data/tarun/celeba_hq_256"

    # Put the whole images in jpeg instead of png
    with open(path_caption, "r") as f:
        res = json.load(f)
    os.makedirs(path_data.replace("./data", "./data"),exist_ok=True)
    for x,y in tqdm(res.items()):
        im1 = Image.open(x).convert('RGB')
        im1.save(x.replace("/data/", "/data/"))
        
    res_ = {x.replace("/data/", "/data/"):y for x,y in res.items()}
    dataset_dict = {
